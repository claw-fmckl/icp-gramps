# Spec 1.2 — Database Schema

## Goal
Define the SQLite database schema for the genealogy data model. Create SQL migration files managed by `ic-sql-migrate`.

## Reference
- [Gramps SQL Database](https://www.gramps-project.org/wiki/index.php/Gramps_SQL_Database) — conceptual reference for entities and relationships
- [promptathon-showcase seeds](https://github.com/kristoferlund/promptathon-showcase) — example of how migrations/seeds are structured

## Migration File

Create `server/src/migrations/001_initial.sql`:
...
- [ ] `server/src/migrations/001_initial.sql` exists and is valid SQL
- [ ] All tables are created without errors when `sqlite3 :memory: < 001_initial.sql` is run
- [ ] Indexes exist for common query patterns (surname, gramps_id, event type, date sort)
- [ ] FTS5 virtual table is present
- [ ] `id_counter` table exists with all entity types pre-populated
